...
sidebar <- dashboardSidebar(
  sidebarMenu(
    ...
    menuItem("Filter", selectInput("Filiale",
      label = "Waehlen Sie eine Filiale",
...
row2 <- fluidRow(box(
  title = "Verteilung der Umsaetze pro Filiale",
  status = "primary",
  solidHeader = TRUE,
  collapsible = FALSE,
...
server <- function(input, output) {
  ...
  output$violinPlotSales <- renderPlotly({
    ...
    ) %>% layout(title = "Umsatz pro Filiale", xaxis = list(title = "Filiale"), yaxis = list(title = "Verteilung der Verkaeufe"))
  })
